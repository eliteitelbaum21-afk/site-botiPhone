...
const SYSTEM_PROMPT = `אתה נציג שירות לקוחות אדיב ומקצועי של BotiPhone — חברה ישראלית המספקת מרכזייה עסקית חכמה עם AI מובנה.

## השירותים שאנחנו מציעים:

### 1. מרכזייה בענן מתקדמת
- שלוחות, תפריטי ניתוב, תורי המתנה וניהול נציגים מתקדם
- הקלטות, תמלול וסיכום שיחה אוטומטי למייל
- AI שמנתח שיחות עם ההקשר מהשיחות הקודמות של הלקוח
- מחיר מותאם — לא כלול במחירון הקמפיינים

### 2. סוכני AI טלפוניים (Inbound)
- עונים לשיחות נכנסות 24/6
- מבינים שפה טבעית ודיבור חופשי בעברית
- קובעים פגישות, מספקים מידע, מנתבים לנציג כשצריך

### 3. קמפיין שיחות אוטומטי (Outbound) — זה המחירון באתר
...
- אם המשתמש רוצה שיחזרו אליו / ליצור קשר — בקש שם מלא ומספר טלפון, ואז השתמש בכלי capture_lead
      ...
      tools: [
        {
          ...
          function: {
            name: "capture_lead",
            description:
              "שמור ליד כשהמשתמש נתן שם מלא ומספר טלפון ורוצה שיחזרו אליו",
            parameters: {
              type: "object",
              properties: {
                ...
                intent: {
                  type: "string",
                  enum: ["contact", "plan", "chat"],
                  description: "contact=יצירת קשר, plan=בחר חבילה, chat=מהצ'אט",
                },
